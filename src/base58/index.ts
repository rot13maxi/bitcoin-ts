...
export function EncodeBase58(input: Uint8Array | readonly number[]): string {
    ...
    // Convert to string
    return output.map(v => base58Encode(v)).join('').split('').reverse().join('');
}

/**
 * Decode a base58 string to byte vector
 */
export function DecodeBase58(str: string, maxRetLen: number = 0): Uint8Array | null {
    ...
    const paddedHex = leadingOnes * 2 + (hexStr.length % 2 === 0 ? 0 : 1) + hexStr.length;
 ...
 */
export function EncodeBase58Check(input: Uint8Array | readonly number[]): string {
    const data = new Uint8Array(input);
 ...
 */
export function encodeBase58Check(version: number, payload: Uint8Array | readonly number[]): string {
