...
export function HexStr(data: Uint8Array | readonly number[], fReverseBytes?: boolean): string {
    let result = '';
...
export function countLeadingZeros(data: Uint8Array | readonly number[]): number {
    let count = 0;
...
export function copySpan(dest: Uint8Array, destOffset: number, src: Uint8Array | readonly number[]): void {
...
export function insert<T>(vec: T[], item: T, comp: (a: T, b: T) => number): void {
...
export function contains<T>(vec: readonly T[], item: T, comp: (a: T, b: T) => number): boolean {
