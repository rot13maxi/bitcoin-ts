 ...
 */
export function encodeScriptHash(
  sh: ScriptHash,
  params: ChainParams = MAINNET,
): string {
 ...
 */
export function encodeWitnessPKHash(
  wpkh: WitnessPKHash,
  params: ChainParams = MAINNET,
): string {
 ...
 */
export function encodeWitnessScriptHash(
  wsh: WitnessScriptHash,
  params: ChainParams = MAINNET,
): string {
...
export function decodeBech32Address(
  ...
  if (decoded.hrp.toLowerCase() !== params.bech32HRP.toLowerCase()) {
    if (errorMsg)
      errorMsg.push(
        `Invalid HRP (expected ${params.bech32HRP}, got ${decoded.hrp})`,
      );
    return new CNoDestination();
  }
  ...
  if (version === 0) {
    ...
    if (errorMsg)
      errorMsg.push(
        `Invalid v0 program size (expected 20 or 32, got ${program.length})`,
      );
    return new CNoDestination();
  }
  ...
  if (program.length < 2 || program.length > 40) {
    if (errorMsg)
      errorMsg.push(`Invalid witness program size (got ${program.length})`);
    return new CNoDestination();
  }
 ...
 */
export function isValidDestinationString(
  str: string,
  params: ChainParams = MAINNET,
): boolean {
