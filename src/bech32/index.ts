...
export function Decode(
  str: string,
  limit: CharLimit = CharLimit.BECH32,
): DecodeResult {
...
export function LocateErrors(
  ...
  // Check checksum
  if (
    !verifyChecksum(Encoding.BECH32, hrp, data) &&
    !verifyChecksum(Encoding.BECH32M, hrp, data)
  ) {
...
export function encodeSegWitAddress(
  ...
  if (version === 0 && witprog.length !== 20 && witprog.length !== 32)
    return "";
