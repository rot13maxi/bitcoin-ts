  ...
  constructor(data?: Uint8Array | readonly number[] | string) {
    ...
    if (data !== undefined) {
      if (typeof data === "string") {
        if (data.length !== 40)
          throw new Error("Hex string must be 40 characters");
        for (let i = 0; i < 20; i++) {
          const hi = HEX_MAP[data[i * 2]] ?? 0;
          const lo = HEX_MAP[data[i * 2 + 1]] ?? 0;
          this.m_data[i] = (hi << 4) | lo;
        }
      } else {
  ...
  constructor(value?: Uint8Array | readonly number[] | string | number) {
    ...
    if (value !== undefined) {
      if (typeof value === "number") {
        ...
        for (let i = 0; i < 32; i++) {
          const hi = HEX_MAP[value[i * 2]] ?? 0;
          const lo = HEX_MAP[value[i * 2 + 1]] ?? 0;
          this.m_data[i] = (hi << 4) | lo;
        }
      } else {
