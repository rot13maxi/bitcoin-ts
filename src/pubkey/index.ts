...
export class XOnlyPubKey {
  ...
  static readonly NUMS_H: XOnlyPubKey = new XOnlyPubKey(
    new uint256(
      "c1c77ade5aba4d81fc2c56a678e6c4ff5a7e7e26ce46c6a31fc91c0a4c2c2d51",
    ),
  );
...
export class CExtPubKey {
  ...
  equals(other: CExtPubKey): boolean {
    ...
    for (let i = 0; i < 4; i++)
      if (this.vchFingerprint[i] !== other.vchFingerprint[i]) return false;
    for (let i = 0; i < 32; i++)
      if (this.chaincode[i] !== other.chaincode[i]) return false;
